...
for iName = 1:length(includedNames)
    ...
    for jj = 1 : size(yPred,2)
        Std(1,jj,iName) = std(yPred(:,jj,iName) - ...
            yDivided(:,jj,iName));
    end
    
    x(:,:,iName) = linspace(0, 1, length(yTest(:,iName))/resolution);
    plot(x(1,:,iName), Std(1,:,iName))
...
for iName = 1: length(includedNames)
    if (iName == 1)
        plot(x(1,:,iName), Std(1,:,iName), 'r')
        hold on
    elseif (iName == 2)
        plot((x(1,:,iName)), ...
            (Std(1,:,iName) + Std(1,:,iName - 1)), 'green')
        hold on
    elseif (iName == 3)
        plot((x(1,:,iName)), ...
            (Std(1,:,iName) + Std(1,:,iName - 1) + ...
            Std(1,:,iName - 2)), 'blue')
        hold on
    else
        plot((x(1,:,iName)), ...
            (Std(1,:,iName) + Std(1,:,iName - 1) + ...
            Std(1,:,iName - 2) + Std(1,:,iName - 3)),'cyan')
        hold on
    end
end
hold off
...
for iName = 1: length(includedNames)
    ...
    elseif (iName == 2)
        plot((x(1,:,iName)), ...
            (sqrt((Std(1,:,iName)).^2 + Std(1,:,iName - 1).^2)))
        hold on
    elseif (iName == 3)
        plot((x(1,:,iName)), ...
            (sqrt(Std(1,:,iName).^2 + Std(1,:,iName - 1).^2 +...
        ...
        plot((x(1,:,iName)), ...
            (sqrt(Std(1,:,iName).^2 + Std(1,:,iName - 1).^2 + ...
            Std(1,:,iName - 2).^2 + Std(1,:,iName - 3).^2)))
        hold on
    end
end
hold off
